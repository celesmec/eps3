...
def rrr(q, n, deg):
	"""
	Calculation expansions for r and their powers (deg).
	- n is maximum degree of variables.
	"""
	from pyranha.math import cos, sin
...
def cosine(p, q, n, deg):
	"""
	Calculation expansions for cosine between two radius vectors and their powers (deg).
	- n is maximum degree of variables.
	"""
...
def check_base(elmass, num = 0, p = 1, q = 2, c_max = 25, deg = 1):
	...
	if num in [0, 4]:
		for n in range(1, 17):
			start = time.time()
			temp = one_delta(p, q, n, c_max, deg, isAver = False, isCalc = True, isPrint = False)
			finish = time.time()-start
			sum_items = 0
			for item in temp.list:
				for jtem in item[0].list:
			...
			print 'n =', str(n).rjust(2, ' '), '|1/d', '%.2f'%finish, str(sum_items).rjust(8, ' '), '%.16e'%dd[str(q)+str(p)]**deg, '%.16e'%evaluated, '%.1e'%accuracy
########################################################################
